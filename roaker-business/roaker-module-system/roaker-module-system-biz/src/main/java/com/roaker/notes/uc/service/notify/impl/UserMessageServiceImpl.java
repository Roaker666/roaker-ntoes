...
import com.roaker.notes.uc.utils.FreemarkerReplaceUtils;
...
public class UserMessageServiceImpl implements UserMessageService {
    ...
    private void doSendMsg(String topic, Object msgPayLoad) {
        try {
            pulsarTemplate.sendAsync(topic, msgPayLoad).whenCompleteAsync((msgId, throwable) -> {
                if (throwable != null) {
                    log.error("Message[{}] -> Pulsar error", msgId, throwable);
                } else {
                    log.info("Message[{}] -> Pulsar success", msgId);
                }
            });
