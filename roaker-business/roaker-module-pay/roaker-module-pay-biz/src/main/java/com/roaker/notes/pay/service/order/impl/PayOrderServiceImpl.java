package com.roaker.notes.pay.service.order.impl;
...
import com.roaker.notes.commons.utils.MoneyUtils;
...
import com.roaker.notes.pay.dal.mapper.order.PayOrderExtensionMapper;
import com.roaker.notes.pay.dal.mapper.order.PayOrderMapper;
import com.roaker.notes.pay.framework.pay.config.PayProperties;
...
import com.roaker.notes.pay.vo.order.PayOrderSubmitReqVO;
...
public class PayOrderServiceImpl implements PayOrderService {
    ...
    public Long createOrder(PayOrderCreateReqDTO reqDTO) {
        ...
        PayOrderDO order = payOrderMapper.selectByAppIdAndMerchantOrderId(reqDTO.getAppId(), reqDTO.getMerchantOrderId());
        if (order != null) {
            log.warn("[createOrder][appId({}) merchantOrderId({}) 已经存在对应的支付单({})]", order.getAppId(),
                    order.getMerchantOrderId(), JacksonUtils.toJSON(order)); // 理论来说，不会出现这个情况
            return order.getId();
        }
        ...
        order = PayOrderConvert.INSTANCE.convert(reqDTO)
                ...
                // 订单相关字段
                .setPayOrderStatus(PayOrderStatusEnum.WAITING)
                // 退款相关字段
                .setRefundPrice(0);
    ...
    private PayOrderDO validateOrderCanSubmit(Long id) {
        ...
        if (PayOrderStatusEnum.SUCCESS == order.getPayOrderStatus()) { // 校验状态，发现已支付
            throw exception(PAY_ORDER_STATUS_IS_SUCCESS);
        }
        if (PayOrderStatusEnum.WAITING != order.getPayOrderStatus()) { // 校验状态，必须是待支付
        ...
        orderExtensions.forEach(orderExtension -> {
            ...
            if (respDTO != null && PayOrderStatusRespEnum.isSuccess(respDTO.getStatus())) {
                log.warn("[validateOrderCanSubmit][order({}) 的 PayOrderRespDTO({}) 已支付，可能是回调延迟]",
                        id, JacksonUtils.toJSON(respDTO));
                throw exception(PAY_ORDER_EXTENSION_IS_PAID);
            }
        });
    }
    ...
    private void notifyOrderSuccess(PayChannelDO channel, PayOrderRespDTO notify) {
        ...
        // 3. 插入支付通知记录
        notifyService.createPayNotifyTask(PayNotifyTypeEnum.ORDER.getCode(), orderExtension.getOrderId());
    }
    ...
    private PayOrderExtensionDO updateOrderSuccess(PayOrderRespDTO notify) {
        ...
        if (PayOrderStatusEnum.SUCCESS == orderExtension.getPayStatus()) { // 如果已经是成功，直接返回，不用重复更新
            log.info("[updateOrderExtensionSuccess][orderExtension({}) 已经是已支付，无需更新]", orderExtension.getId());
            return orderExtension;
        }
        if (ObjectUtil.notEqual(orderExtension.getPayStatus(), PayOrderStatusEnum.WAITING)) { // 校验状态，必须是待支付
        ...
        int updateCounts = orderExtensionMapper.updateByIdAndStatus(
                ...
                orderExtension.getPayStatus().getCode(),
                PayOrderExtensionDO.builder()
                        .payStatus(PayOrderStatusEnum.SUCCESS)
                        .channelNotifyData(JacksonUtils.toJSON(notify)).build());
    ...
    private Boolean updateOrderSuccess(PayChannelDO channel, PayOrderExtensionDO orderExtension,
        ...
        if (PayOrderStatusEnum.SUCCESS == order.getPayOrderStatus() // 如果已经是成功，直接返回，不用重复更新
        ...
        // 2. 更新 PayOrderDO
        int updateCounts = payOrderMapper.updateByIdAndStatus(
                order.getId(),
                PayOrderStatusEnum.WAITING.getCode(),
                PayOrderDO.builder()
                        .payOrderStatus(PayOrderStatusEnum.SUCCESS)
                        .channelId(channel.getId())
                        .channelCode(channel.getCode())
                        .successTime(notify.getSuccessTime())
                        .extensionId(orderExtension.getId())
                        .no(orderExtension.getNo())
    ...
    private void updateOrderExtensionClosed(PayChannelDO channel, PayOrderRespDTO notify) {
        ...
        if (PayOrderStatusEnum.CLOSED == orderExtension.getPayStatus()) { // 如果已经是关闭，直接返回，不用重复更新
        ...
        int updateCounts = orderExtensionMapper.updateByIdAndStatus(orderExtension.getId(),
                orderExtension.getPayStatus().getCode(),
                PayOrderExtensionDO.builder()
                        .payStatus(PayOrderStatusEnum.CLOSED)
                        .channelNotifyData(JacksonUtils.toJSON(notify))
                        .channelErrorCode(notify.getChannelErrorCode())
                        .channelErrorMsg(notify.getChannelErrorMsg()).build());
        if (updateCounts == 0) { // 校验状态，必须是待支付
            throw exception(PAY_ORDER_EXTENSION_STATUS_IS_NOT_WAITING);
        }
    ...
    public void updateOrderRefundPrice(Long id, Integer incrRefundPrice) {
        ...
        if (!PayOrderStatusEnum.isSuccessOrRefund(order.getPayOrderStatus().getCode())) {
        ...
        PayOrderDO updateObj = new PayOrderDO()
                ...
                .setPayOrderStatus(PayOrderStatusEnum.REFUND);
        int updateCount = payOrderMapper.updateByIdAndStatus(id, order.getPayOrderStatus().getCode(), updateObj);
        if (updateCount == 0) {
            throw exception(PAY_ORDER_REFUND_FAIL_STATUS_ERROR);
        }
    }
    ...
    public void updatePayOrderPrice(Long id, Integer payPrice) {
        ...
        if (ObjectUtil.notEqual(PayOrderStatusEnum.WAITING, order.getPayOrderStatus())) {
            throw exception(PAY_ORDER_STATUS_IS_NOT_WAITING);
        }
    ...
    public int syncOrder(LocalDateTime minCreateTime) {
        ...
        List<PayOrderExtensionDO> orderExtensions = orderExtensionMapper.selectListByStatusAndCreateTimeGe(
                PayOrderStatusEnum.WAITING.getCode(), minCreateTime);
    ...
    private boolean expireOrder(PayOrderDO order) {
        try {
            ...
            for (PayOrderExtensionDO orderExtension : orderExtensions) {
                ...
                if (PayOrderStatusRespEnum.isRefund(respDTO.getStatus())) {
                    ...
                    log.error("[expireOrder][extension({}) 的 PayOrderRespDTO({}) 已退款，可能是回调延迟]",
                            orderExtension.getId(), JacksonUtils.toJSON(respDTO));
                    return false;
                }
                ...
                // 兜底逻辑：将支付拓展单更新为已关闭
                PayOrderExtensionDO updateObj = new PayOrderExtensionDO().setPayStatus(PayOrderStatusEnum.CLOSED)
                        .setChannelNotifyData(JacksonUtils.toJSON(respDTO));
                if (orderExtensionMapper.updateByIdAndStatus(orderExtension.getId(), PayOrderStatusEnum.WAITING.getCode(),
                        updateObj) == 0) {
            ...
            PayOrderDO updateObj = new PayOrderDO().setPayOrderStatus(PayOrderStatusEnum.CLOSED);
            if (payOrderMapper.updateByIdAndStatus(order.getId(), order.getPayOrderStatus().getCode(), updateObj) == 0) {
