package com.roaker.notes.pay.service.refund.impl;
...
import com.roaker.notes.pay.dal.mapper.refund.PayRefundMapper;
...
public class PayRefundServiceImpl implements PayRefundService {
    ...
    public Long createPayRefund(PayRefundCreateReqDTO reqDTO) {
        ...
        // 2.1 插入退款单
        String no = seqApi.getSegmentId(payProperties.getRefundNoPrefix());
        refund = PayRefundConvert.INSTANCE.convert(reqDTO)
                ...
                // 退款相关字段
                .setPayRefundStatus(PayRefundStatusEnum.WAITING)
    ...
    private PayOrderDO validatePayOrderCanRefund(PayRefundCreateReqDTO reqDTO) {
        ...
        if (refundMapper.selectCountByAppIdAndOrderId(reqDTO.getAppId(), order.getId(),
                PayRefundStatusEnum.WAITING.getCode()) > 0) {
    ...
    private void notifyRefundSuccess(PayChannelDO channel, PayRefundRespDTO notify) {
        ...
        if (PayRefundStatusEnum.SUCCESS == refund.getPayRefundStatus()) { // 如果已经是成功，直接返回，不用重复更新
            log.info("[notifyRefundSuccess][退款订单({}) 已经是退款成功，无需更新]", refund.getId());
            return;
        }
        if (PayRefundStatusEnum.WAITING != refund.getPayRefundStatus()) {
        ...
        PayRefundDO updateRefundObj = new PayRefundDO()
                ...
                .setPayRefundStatus(PayRefundStatusEnum.SUCCESS)
                .setChannelNotifyData(JacksonUtils.toJSON(notify));
        int updateCounts = refundMapper.updateByIdAndStatus(refund.getId(), refund.getPayRefundStatus().getCode(), updateRefundObj);
        ...
        // 3. 插入退款通知记录
        notifyService.createPayNotifyTask(PayNotifyTypeEnum.REFUND.getCode(),
                refund.getId());
    }
    ...
    private void notifyRefundFailure(PayChannelDO channel, PayRefundRespDTO notify) {
        ...
        if (PayRefundStatusEnum.FAILURE == refund.getPayRefundStatus()) { // 如果已经是成功，直接返回，不用重复更新
        ...
        PayRefundDO updateRefundObj = new PayRefundDO()
                .setChannelRefundNo(notify.getChannelRefundNo())
                .setPayRefundStatus(PayRefundStatusEnum.FAILURE)
                .setChannelNotifyData(JacksonUtils.toJSON(notify))
        ...
        // 2. 插入退款通知记录
        notifyService.createPayNotifyTask(PayNotifyTypeEnum.REFUND.getCode(), refund.getId());
    }

    @Override
    public int syncRefund() {
        // 1. 查询指定创建时间内的待退款订单
        List<PayRefundDO> refunds = refundMapper.selectListByStatus(PayRefundStatusEnum.WAITING.getCode());
