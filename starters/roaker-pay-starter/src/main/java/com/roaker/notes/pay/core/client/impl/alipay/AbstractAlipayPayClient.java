...
import static com.roaker.notes.exception.enums.GlobalErrorCodeConstants.*;
...
public abstract class AbstractAlipayPayClient extends AbstractPayClient<AlipayPayClientConfig> {
    ...
    protected PayTransferRespDTO doUnifiedTransfer(PayTransferUnifiedReqDTO reqDTO) throws Throwable {
        ...
        if (reqDTO.getChannelExtras() != null) {
            model.setBusinessParams(JacksonUtils.toJSON(reqDTO.getChannelExtras()));
        }
        // ② 个性化的参数
        Participant payeeInfo = new Participant();
        PayTransferTypeEnum transferType = PayTransferTypeEnum.typeOf(reqDTO.getType());
        switch (transferType) {
            // TODO:是不是不用传递 transferType 参数哈？因为应该已经明确是支付宝啦？
            // 是不是还要考虑转账到银行卡。所以传 transferType 但是转账到银行卡不知道要如何测试??
            case ALIPAY_BALANCE: {
