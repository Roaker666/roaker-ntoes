...
public class AlipayPcPayClient extends AbstractAlipayPayClient{
    public AlipayPcPayClient(Long channelId, AlipayPayClientConfig config) {
        super(channelId, PayChannelEnum.ALIPAY_PC.getChannelCode(), config);
    }
    ...
    protected PayOrderRespDTO doUnifiedOrder(PayOrderUnifiedReqDTO reqDTO) throws Throwable {
        ...
        String displayMode = ObjectUtil.defaultIfNull(reqDTO.getDisplayMode(), PayOrderDisplayModeEnum.URL.getName());
        ...
        if (Objects.equals(displayMode, PayOrderDisplayModeEnum.FORM.getName())) {
