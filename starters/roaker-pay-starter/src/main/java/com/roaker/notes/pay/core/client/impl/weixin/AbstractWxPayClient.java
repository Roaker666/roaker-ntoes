...
import static com.roaker.notes.pay.core.client.impl.weixin.WxPayClientConfig.API_VERSION_V2;
...
//        payConfig.setCertSerialNo();
    ...
    private PayOrderRespDTO doParseOrderNotifyV2(String body) throws WxPayException {
        ...
        Integer status = Objects.equals(response.getResultCode(), "SUCCESS") ?
                PayOrderStatusRespEnum.SUCCESS.getCode() : PayOrderStatusRespEnum.CLOSED.getCode();
    ...
    private static Integer parseStatus(String tradeState) {
        return switch (tradeState) {
            case "NOTPAY", "USERPAYING" -> // 支付中，等待用户输入密码（条码支付独有）
                    PayOrderStatusRespEnum.WAITING.getCode();
            case "SUCCESS" -> PayOrderStatusRespEnum.SUCCESS.getCode();
            case "REFUND" -> PayOrderStatusRespEnum.REFUND.getCode(); // 已撤销（刷卡支付独有）
            case "CLOSED", "REVOKED", "PAYERROR" -> // 支付失败（其它原因，如银行返回失败）
                    PayOrderStatusRespEnum.CLOSED.getCode();
